...
class XMacro {
        ...
        for (field in fields) {
            ...
            switch field.kind {
                ...
                case FFun(f):
                    ...
                    if (hasXMeta(field)) {
                        stateFields = transformMarkupComponent(field);
                    }
    ...
    static function transformMarkupComponent(field:Field):Map<String,FunctionArg> {
        ...
        switch field.kind {
            ...
            case FFun(fn):
                ...
                fn.args = [{
                    name: 'xid_',
                    type: macro :markup_.Xid
                },
                {
                    name: 'ctx_',
                    type: macro :markup_.ReactiveContext
                },
                {
                    name: 'data_',
                    type: macro :markup_.VNodeData
                },
                {
                    name: 'children',
                    type: macro :Array<markup_.VNodeData>
                }];
                #end
    ...
    static function processInlineMarkup(e:Expr):Expr {
    ...
    static function processMarkupString(s:String, kind:StringLiteralKind, pos:Position):Expr {
        if (!macroHasErrors) {
            ...
            try {
                ...
                #if !completion
                res = '{markup_.Markup.begin(); final vdom_ = $res; markup_.Markup.end(); vdom_;}';
                #end
